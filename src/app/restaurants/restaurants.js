export const restaurants = [
  {
    id: "rest_001",
    slug: "kacchi-bhai-dhanmondi",
    name: "Kacchi Bhai",
    logo: "https://i.ibb.co/1YFN5k3C/kacchi-bhai-2-logo.png",
    banner:
      "https://i.ibb.co/mCVYhy44/177534213-311121510627014-6362311441237242299-n.jpg",
    rating: 4.6,
    reviewsCount: 128,
    cuisines: ["Bengali", "Biriyani"],
    priceRange: "৳৳",
    estimatedDeliveryTime: "30-40 min",
    deliveryFee: 30,
    status: "approved",
    isFeatured: true,
    isActive: true,
    approved: true,
    ownerId: "user_rest_01",
    ownerEmail: "xyz@gmail.com",
    location: {
      address: "House 12/A, Road 5, Dhanmondi",
      area: "Dhanmondi",
      city: "Dhaka",
      country: "Bangladesh",
      coordinates: { lat: 23.7465, lng: 90.376 },
    },
    contact: {
      phone: "01710000001",
      email: "info@kacchibhai.bd",
    },
    openingHours: {
      mon: { open: "10:00", close: "23:00" },
      tue: { open: "10:00", close: "23:00" },
      wed: { open: "10:00", close: "23:00" },
      thu: { open: "10:00", close: "23:00" },
      fri: { open: "12:00", close: "23:00" },
      sat: { open: "12:00", close: "22:00" },
      sun: { open: "10:00", close: "22:00" },
    },

    menu: [
      {
        id: "m_kb_01",
        name: "Kacchi Biriyani",
        description:
          "Traditional slow-cooked mutton biriyani with saffron and spices.",
        price: 250,
        category: "Main",
        image: "https://i.ibb.co/hJzDK3Xr/kacchi-biriyani.png",
        massUnit: "plate",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_kb_02",
        name: "Borhani",
        description: "Spiced yogurt drink, perfect with biriyani.",
        price: 60,
        category: "Beverage",
        image: "https://i.ibb.co/JR1NPWXN/borhani.png",
        massUnit: "cup",
        isAvailable: true,
        discount: { percent: 5, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_kb_03",
        name: "Mixed Kabab Platter",
        description: "Chicken & beef kababs served with naan and chutney.",
        price: 320,
        category: "Share",
        image: "https://i.ibb.co/F4rbd86T/mixed-kebab.png",
        massUnit: "plate",
        isAvailable: true,
        discount: { percent: 10, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
    ],
    stats: {
      totalOrders: 4520,
      monthlyEarnings: 1250000,
    },
    tags: ["Popular", "Family"],
    createdAt: "2025-03-10T09:30:00Z",
    updatedAt: "2025-09-10T12:00:00Z",
  },

  {
    id: "rest_002",
    slug: "pizza-point-gulshan",
    name: "Pizza Point",
    logo: "https://i.ibb.co/WWxNbFgt/pizza-point-3.png",
    banner:
      "https://i.ibb.co/Ps520SVK/ba4249fb-fde9-4acf-8352-b158850379f9.jpg",
    rating: 4.3,
    reviewsCount: 89,
    cuisines: ["Italian", "Fast Food"],
    priceRange: "৳৳৳",
    estimatedDeliveryTime: "25-35 min",
    deliveryFee: 40,
    status: "approved",
    isFeatured: false,
    isActive: true,
    approved: true,
    ownerId: "user_rest_02",
    ownerEmail: "afroza@gmail.com",
    location: {
      address: "House 5, Road 11, Gulshan-2",
      area: "Gulshan",
      city: "Dhaka",
      country: "Bangladesh",
      coordinates: { lat: 23.7925, lng: 90.4078 },
    },
    contact: {
      phone: "01710000002",
      email: "hello@pizzapoint.bd",
    },
    openingHours: {
      mon: { open: "12:00", close: "23:00" },
      tue: { open: "12:00", close: "23:00" },
      wed: { open: "12:00", close: "23:00" },
      thu: { open: "12:00", close: "23:00" },
      fri: { open: "12:00", close: "23:00" },
      sat: { open: "12:00", close: "22:00" },
      sun: { open: "12:00", close: "22:00" },
    },

    menu: [
      {
        id: "m_pp_01",
        name: "Cheese Pizza",
        description:
          "Classic hand-tossed pizza with mozzarella and special sauce.",
        price: 450,
        category: "Pizza",
        image: "https://i.ibb.co/Q3M7jPLZ/cheese-pizza.png",
        massUnit: "medium",
        isAvailable: true,
        discount: { percent: 5, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_pp_02",
        name: "Garlic Bread",
        description: "Buttery garlic bread, perfect side for pizzas.",
        price: 150,
        category: "Sides",
        image: "https://i.ibb.co/VYs41fsX/garlic-bread.png",
        massUnit: "plate",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_pp_03",
        name: "Pepperoni Pizza",
        description: "Loaded with pepperoni and cheese.",
        price: 520,
        category: "Pizza",
        image: "https://i.ibb.co/mMkzX5Q/pepperoni-pizza.png",
        massUnit: "medium",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
    ],
    stats: {
      totalOrders: 2980,
      monthlyEarnings: 980000,
    },
    tags: ["Fast Delivery", "Hot"],
    createdAt: "2024-11-20T08:15:00Z",
    updatedAt: "2025-09-05T10:00:00Z",
  },

  {
    id: "rest_003",
    slug: "banani-bites",
    name: "Banani Bites",
    logo: "https://i.ibb.co/7t409KqN/banani-bites-logo.png",
    banner:
      "https://i.ibb.co/0RcNNC9q/Rolla-Sharjah-restaurants-021120.jpg",
    rating: 4.8,
    reviewsCount: 204,
    cuisines: ["Dessert", "Cafe"],
    priceRange: "৳",
    estimatedDeliveryTime: "15-25 min",
    deliveryFee: 0,
    status: "approved",
    isFeatured: true,
    isActive: true,
    approved: true,
    ownerId: "user_rest_03",
    ownerEmail: "nipa@gmail.com",
    location: {
      address: "Road 2, Banani",
      area: "Banani",
      city: "Dhaka",
      country: "Bangladesh",
      coordinates: { lat: 23.7929, lng: 90.41 },
    },
    contact: {
      phone: "01710000003",
      email: "contact@bananibites.bd",
    },
    openingHours: {
      mon: { open: "11:00", close: "23:00" },
      tue: { open: "11:00", close: "23:00" },
      wed: { open: "11:00", close: "23:00" },
      thu: { open: "11:00", close: "23:00" },
      fri: { open: "11:00", close: "23:00" },
      sat: { open: "11:00", close: "23:00" },
      sun: { open: "12:00", close: "22:00" },
    },

    menu: [
      {
        id: "m_bb_01",
        name: "Chocolate Cake",
        description: "Warm molten chocolate with vanilla ice cream.",
        price: 180,
        category: "Dessert",
        image: "https://i.ibb.co/tMC0zhRw/chocolate-cake.png",
        massUnit: "piece",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_bb_02",
        name: "Cappuccino",
        description: "Classic espresso-based drink with steamed milk.",
        price: 120,
        category: "Beverage",
        image: "https://i.ibb.co/v49NG7xq/cappuccino.png",
        massUnit: "cup",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_bb_03",
        name: "Blueberry Cheesecake",
        description: "Rich cheesecake topped with blueberry compote.",
        price: 220,
        category: "Dessert",
        image: "https://i.ibb.co/CsTKM5hr/blueberry-cheesecake.png",
        massUnit: "slice",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
    ],
    stats: {
      totalOrders: 5120,
      monthlyEarnings: 1420000,
    },
    tags: ["Quick", "Low Price"],
    createdAt: "2025-01-05T11:20:00Z",
    updatedAt: "2025-09-12T09:45:00Z",
  },
  {
    id: "rest_004",
    slug: "burger-hub-mirpur",
    name: "Burger Hub",
    logo: "https://i.ibb.co/tpHtwyVq/burger-hub-logo.png",
    banner:
      "https://i.ibb.co/wZsXm631/774f65184299217-Y3-Jvc-Cwx-MDgw-LDg0-NCww-LDEx-Nw.jpg",
    rating: 4.5,
    reviewsCount: 175,
    cuisines: ["Fast Food", "Burgers"],
    priceRange: "৳৳",
    estimatedDeliveryTime: "20-30 min",
    deliveryFee: 25,
    status: "approved",
    isFeatured: true,
    isActive: true,
    approved: true,
    ownerId: "user_rest_04",
    ownerEmail: "julkarnain@gmail.com",
    location: {
      address: "Block B, Section 11, Mirpur",
      area: "Mirpur",
      city: "Dhaka",
      country: "Bangladesh",
      coordinates: { lat: 23.8151, lng: 90.3666 },
    },
    contact: {
      phone: "01710000004",
      email: "support@burgerhub.bd",
    },
    openingHours: {
      mon: { open: "11:00", close: "23:00" },
      tue: { open: "11:00", close: "23:00" },
      wed: { open: "11:00", close: "23:00" },
      thu: { open: "11:00", close: "23:00" },
      fri: { open: "11:00", close: "23:00" },
      sat: { open: "11:00", close: "23:00" },
      sun: { open: "12:00", close: "22:00" },
    },

    menu: [
      {
        id: "m_bh_01",
        name: "Classic Beef Burger",
        description: "Juicy beef patty with lettuce, tomato & sauce.",
        price: 220,
        category: "Burger",
        image: "https://i.ibb.co/yF1tgSqd/classic-beef-burger.png",
        massUnit: "piece",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: true, type: "Buy1Get1", freeQty: 1 },
      },
      {
        id: "m_bh_02",
        name: "Cheese Fries",
        description: "Crispy fries topped with melted cheese.",
        price: 120,
        category: "Sides",
        image: "https://i.ibb.co/gMH24tVV/cheese-fries.png",
        massUnit: "plate",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_bh_03",
        name: "Chicken Zinger",
        description: "Crispy chicken fillet burger with mayo.",
        price: 200,
        category: "Burger",
        image: "https://i.ibb.co/B5jb1s2T/chicken-zinger.png",
        massUnit: "piece",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
    ],
    stats: {
      totalOrders: 3780,
      monthlyEarnings: 865000,
    },
    tags: ["Popular", "Spicy"],
    createdAt: "2024-12-10T10:30:00Z",
    updatedAt: "2025-09-15T08:20:00Z",
  },

  {
    id: "rest_005",
    slug: "thai-treats-uttara",
    name: "Time for Thai",
    logo: "https://i.ibb.co/Z1LxGTdk/time-for-thai-logo.png",
    banner: "https://i.ibb.co/mF5BspK7/timeforthai-mobile.jpg",
    rating: 4.4,
    reviewsCount: 143,
    cuisines: ["Thai", "Asian"],
    priceRange: "৳৳৳",
    estimatedDeliveryTime: "35-45 min",
    deliveryFee: 50,
    status: "approved",
    isFeatured: false,
    isActive: true,
    approved: true,
    ownerId: "user_rest_05",
    ownerEmail: "jerin@gmail.com",
    location: {
      address: "Sector 9, Uttara",
      area: "Uttara",
      city: "Dhaka",
      country: "Bangladesh",
      coordinates: { lat: 23.8767, lng: 90.3796 },
    },
    contact: {
      phone: "01710000005",
      email: "info@thaitreats.bd",
    },
    openingHours: {
      mon: { open: "12:00", close: "23:00" },
      tue: { open: "12:00", close: "23:00" },
      wed: { open: "12:00", close: "23:00" },
      thu: { open: "12:00", close: "23:00" },
      fri: { open: "12:00", close: "23:00" },
      sat: { open: "12:00", close: "22:00" },
      sun: { open: "12:00", close: "22:00" },
    },

    menu: [
      {
        id: "m_tt_01",
        name: "Pad Thai",
        description: "Stir-fried rice noodles with shrimp and peanuts.",
        price: 480,
        category: "Main",
        image: "https://i.ibb.co/gZqtqMrK/pad-thai.png",
        massUnit: "plate",
        isAvailable: true,
        discount: { percent: 10, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_tt_02",
        name: "Tom Yum Soup",
        description: "Spicy-sour soup with shrimp & herbs.",
        price: 350,
        category: "Soup",
        image: "https://i.ibb.co/Zpc7nmcc/chicken-tom-yum-soup.png",
        massUnit: "bowl",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: true, type: "Buy1Get1", freeQty: 1 },
      },
      {
        id: "m_tt_03",
        name: "Thai Green Curry",
        description: "Coconut milk curry with chicken and vegetables.",
        price: 520,
        category: "Main",
        image: "https://i.ibb.co/4wQ2SHS3/green-curry.png",
        massUnit: "plate",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
    ],
    stats: {
      totalOrders: 2150,
      monthlyEarnings: 645000,
    },
    tags: ["Authentic", "Spicy"],
    createdAt: "2025-02-15T09:45:00Z",
    updatedAt: "2025-09-11T11:10:00Z",
  },

  {
    id: "rest_006",
    slug: "kebab-korner-bashundhara",
    name: "Kebab Korner",
    logo: "https://i.ibb.co/m5Dyyhr1/kebab-korner-logo.png",
    banner: "https://i.ibb.co/GwK6xFc/1663654013-Bg-stevenage.webp",
    rating: 4.2,
    reviewsCount: 97,
    cuisines: ["Middle Eastern", "Grill"],
    priceRange: "৳৳",
    estimatedDeliveryTime: "25-35 min",
    deliveryFee: 30,
    status: "approved",
    isFeatured: false,
    isActive: true,
    approved: true,
    ownerId: "user_rest_06",
    ownerEmail: "imran@gmail.com",
    location: {
      address: "Block C, Bashundhara R/A",
      area: "Bashundhara",
      city: "Dhaka",
      country: "Bangladesh",
      coordinates: { lat: 23.8155, lng: 90.4281 },
    },
    contact: {
      phone: "01710000006",
      email: "order@kebabkorner.bd",
    },
    openingHours: {
      mon: { open: "12:00", close: "23:00" },
      tue: { open: "12:00", close: "23:00" },
      wed: { open: "12:00", close: "23:00" },
      thu: { open: "12:00", close: "23:00" },
      fri: { open: "12:00", close: "23:00" },
      sat: { open: "12:00", close: "23:30" },
      sun: { open: "12:00", close: "23:30" },
    },

    menu: [
      {
        id: "m_kk_01",
        name: "Beef Seekh Kebab",
        description: "Grilled minced beef skewers with spices.",
        price: 300,
        category: "Grill",
        image: "https://i.ibb.co/6JFGDpzr/beef-sheek-kebab.png",
        massUnit: "plate",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_kk_02",
        name: "Chicken Shawarma",
        description: "Tender chicken wrap with garlic sauce.",
        price: 200,
        category: "Wrap",
        image: "https://i.ibb.co/2Y1LDN5g/chicken-shwrma.png",
        massUnit: "roll",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: true, type: "Buy1Get1", freeQty: 1 },
      },
      {
        id: "m_kk_03",
        name: "Falafel Plate",
        description: "Crispy falafels served with hummus.",
        price: 180,
        category: "Veg",
        image: "https://i.ibb.co/9HDLqK08/falafel-plate.png",
        massUnit: "plate",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
    ],
    stats: {
      totalOrders: 1670,
      monthlyEarnings: 420000,
    },
    tags: ["Grill", "Halal"],
    createdAt: "2025-03-01T08:30:00Z",
    updatedAt: "2025-09-09T12:25:00Z",
  },

  {
    id: "rest_007",
    slug: "sushi-senpai-banani",
    name: "Sushi Senpai",
    logo: "https://i.ibb.co/pBsFLPkd/sushi-senpai-logo.png",
    banner:
      "https://i.ibb.co/HLyBLcxY/f4710469-8ac4-40dc-a1d0-f6982ca651f8-1.webp",
    rating: 4.7,
    reviewsCount: 155,
    cuisines: ["Japanese", "Sushi"],
    priceRange: "৳৳৳৳",
    estimatedDeliveryTime: "40-50 min",
    deliveryFee: 60,
    status: "approved",
    isFeatured: true,
    isActive: true,
    approved: true,
    ownerId: "user_rest_07",
    ownerEmail: "kanak@gmail.com",
    location: {
      address: "House 20, Road 12, Banani",
      area: "Banani",
      city: "Dhaka",
      country: "Bangladesh",
      coordinates: { lat: 23.7937, lng: 90.4041 },
    },
    contact: {
      phone: "01710000007",
      email: "contact@sushisenpai.bd",
    },
    openingHours: {
      mon: { open: "12:00", close: "22:30" },
      tue: { open: "12:00", close: "22:30" },
      wed: { open: "12:00", close: "22:30" },
      thu: { open: "12:00", close: "22:30" },
      fri: { open: "13:00", close: "22:00" },
      sat: { open: "13:00", close: "22:00" },
      sun: { open: "12:00", close: "21:30" },
    },

    menu: [
      {
        id: "m_ss_01",
        name: "California Roll",
        description: "Crab, avocado, cucumber sushi rolls.",
        price: 650,
        category: "Sushi",
        image: "https://i.ibb.co/0dt2HFM/california-roll.png",
        massUnit: "8 pcs",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_ss_02",
        name: "Salmon Nigiri",
        description: "Fresh salmon on vinegared rice.",
        price: 550,
        category: "Sushi",
        image: "https://i.ibb.co/rKct9KyW/salmon-nigiri.png",
        massUnit: "2 pcs",
        isAvailable: true,
        discount: { percent: 10, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_ss_03",
        name: "Tempura Udon",
        description: "Thick wheat noodles with tempura prawns.",
        price: 700,
        category: "Noodles",
        image: "https://i.ibb.co/r2v3k6B7/shrimp-tempura-udon.png",
        massUnit: "bowl",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
    ],
    stats: {
      totalOrders: 2450,
      monthlyEarnings: 1180000,
    },
    tags: ["Premium", "Authentic"],
    createdAt: "2025-04-12T09:15:00Z",
    updatedAt: "2025-09-14T11:00:00Z",
  },

  {
    id: "rest_008",
    slug: "roll-express-mohakhali",
    name: "Roll Express",
    logo: "https://i.ibb.co/PG8Y7wnm/roll-express-logo.png",
    banner:
      "https://i.ibb.co/7JFR57nd/roll-express-chandigarh-sector-36d-chandigarh-fast-food-19mf5tu46g.jpg",
    rating: 4.1,
    reviewsCount: 110,
    cuisines: ["Street Food", "Rolls"],
    priceRange: "৳",
    estimatedDeliveryTime: "15-20 min",
    deliveryFee: 20,
    isFeatured: false,
    isActive: true,
    approved: true,
    ownerId: "user_rest_08",
    ownerEmail: "mustakim@gmail.com",
    location: {
      address: "Mohakhali DOHS Gate",
      area: "Mohakhali",
      city: "Dhaka",
      country: "Bangladesh",
      coordinates: { lat: 23.7805, lng: 90.3982 },
    },
    contact: {
      phone: "01710000008",
      email: "order@rollexpress.bd",
    },
    openingHours: {
      mon: { open: "10:00", close: "22:00" },
      tue: { open: "10:00", close: "22:00" },
      wed: { open: "10:00", close: "22:00" },
      thu: { open: "10:00", close: "22:00" },
      fri: { open: "10:00", close: "23:00" },
      sat: { open: "10:00", close: "23:00" },
      sun: { open: "10:00", close: "21:00" },
    },

    menu: [
      {
        id: "m_re_01",
        name: "Beef Roll",
        description: "Paratha roll stuffed with spicy beef.",
        price: 90,
        category: "Roll",
        image: "https://i.ibb.co/s97vBWfy/chicken-roll.png",
        massUnit: "piece",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: true, type: "Buy1Get1", freeQty: 1 },
      },
      {
        id: "m_re_02",
        name: "Chicken Roll",
        description: "Crispy chicken wrapped in paratha.",
        price: 80,
        category: "Roll",
        image: "https://i.ibb.co/s97vBWfy/chicken-roll.png",
        massUnit: "piece",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
      {
        id: "m_re_03",
        name: "Veg Roll",
        description: "Healthy roll with veggies & spices.",
        price: 60,
        category: "Roll",
        image: "https://i.ibb.co/s97vBWfy/chicken-roll.png",
        massUnit: "piece",
        isAvailable: true,
        discount: { percent: 0, validTill: "2025-10-30" },
        bogo: { active: false, type: null, freeQty: 0 },
      },
    ],
    stats: {
      totalOrders: 3200,
      monthlyEarnings: 375000,
    },
    tags: ["Quick", "Budget"],
    createdAt: "2025-03-20T07:50:00Z",
    updatedAt: "2025-09-16T12:40:00Z",
  },
];
