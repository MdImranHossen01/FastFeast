  ...
  useEffect(() => {
    const fetchJapaneseMenus = async () => {
      try {
        ...
        const filteredJapaneseMenus = menus.filter((menu) => menu.cuisine === "Japanese");
        
        setJapaneseMenus(filteredJapaneseMenus);
        console.log("Japanese menus:", filteredJapaneseMenus);
      } catch (err) {
        setError(err.message);
        console.error("Error fetching Japanese menus:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <section className="mb-12 relative">
        ...
        <div className="text-center py-8">
          <p className="text-gray-500">Loading Japanese dishes...</p>
        </div>
      </section>
    );
  }
