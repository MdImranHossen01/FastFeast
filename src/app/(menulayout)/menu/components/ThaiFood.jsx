  ...
  useEffect(() => {
    const fetchThaiMenus = async () => {
      try {
        ...
        const filteredThaiMenus = menus.filter((menu) => menu.cuisine === "Thai");
        
        setThaiMenus(filteredThaiMenus);
        console.log("Thai menus:", filteredThaiMenus);
      } catch (err) {
        setError(err.message);
        console.error("Error fetching Thai menus:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <section className="mb-12 relative">
        ...
        <div className="text-center py-8">
          <p className="text-gray-500">Loading Thai dishes...</p>
        </div>
      </section>
    );
  }
