  ...
  useEffect(() => {
    const fetchTurkishMenus = async () => {
      try {
        ...
        const filteredTurkishMenus = menus.filter((menu) => menu.cuisine === "Turkish");
        
        setTurkishMenus(filteredTurkishMenus);
        console.log("Turkish menus:", filteredTurkishMenus);
      } catch (err) {
        setError(err.message);
        console.error("Error fetching Turkish menus:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <section className="mb-12 relative">
        ...
        <div className="text-center py-8">
          <p className="text-gray-500">Loading Turkish dishes...</p>
        </div>
      </section>
    );
  }
