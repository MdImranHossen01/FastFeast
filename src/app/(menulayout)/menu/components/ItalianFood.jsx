  ...
  useEffect(() => {
    const fetchItalianMenus = async () => {
      try {
        ...
        const filteredItalianMenus = menus.filter((menu) => menu.cuisine === "Italian");
        
        setItalianMenus(filteredItalianMenus);
        console.log("Italian menus:", filteredItalianMenus);
      } catch (err) {
        setError(err.message);
        console.error("Error fetching Italian menus:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <section className="mb-12 relative">
        ...
        <div className="text-center py-8">
          <p className="text-gray-500">Loading Italian dishes...</p>
        </div>
      </section>
    );
  }
