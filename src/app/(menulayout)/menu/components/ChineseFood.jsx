  ...
  useEffect(() => {
    const fetchChineseMenus = async () => {
      try {
        ...
        const filteredChineseMenus = menus.filter((menu) => menu.cuisine === "Chinese");
        
        setChineseMenus(filteredChineseMenus);
        console.log("Chinese menus:", filteredChineseMenus);
      } catch (err) {
        setError(err.message);
        console.error("Error fetching Chinese menus:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <section className="mb-12 relative">
        ...
        <div className="text-center py-8">
          <p className="text-gray-500">Loading Chinese dishes...</p>
        </div>
      </section>
    );
  }
